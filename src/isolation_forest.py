...
from Utils.utils import output_report

# directory
RAW_PATH = 'dataset/isolation_forest_data/raw/'
FINAL_PATH = 'dataset/isolation_forest_data/final/'
OUTPUT_PATH = 'outputs/isolation_forest/'
MODEL_PATH = 'models/isolation_forest/'



df_dataset_1 = pd.read_csv(FINAL_PATH + "training_dataset_1.csv")
df_dataset_2 = pd.read_csv(FINAL_PATH + "training_dataset_2.csv")
df_testing_dataset = pd.read_csv(FINAL_PATH + "testing_dataset.csv")
...
joblib.dump(isoForest_dataset_1, MODEL_PATH + 'spam_classifier_model_1.pkl')
joblib.dump(vectorizer_1, MODEL_PATH + 'tfidf_vectorizer_1.pkl')

# Save model 2 components
joblib.dump(isoForest_dataset_2, MODEL_PATH + 'spam_classifier_model_2.pkl')
joblib.dump(vectorizer_2, MODEL_PATH + 'tfidf_vectorizer_2.pkl')



# Save to text files
with open(OUTPUT_PATH + 'eval_dataset_1_45percent.txt', 'w') as f1:
    f1.write("=== Evaluation for Dataset 1 (45% spam) ===\n")
    f1.write(df_report_1.to_string())

with open(OUTPUT_PATH + 'eval_dataset_2_40percent.txt', 'w') as f2:
